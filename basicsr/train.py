...
def parse_options(is_train=True):
    ...
    parser.add_argument(
        '--opt', type=str, default='configs/full_300k/asanet_lolv1.yml', help='Path to option YAML file.')
    parser.add_argument(
        '--launcher',
...
def create_train_val_dataloader(opt, logger):  # build both loaders
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        if phase == 'train':
            ...
            train_sampler = EnlargedSampler(train_set, opt['world_size'],
                                            opt['rank'], dataset_enlarge_ratio)     # world_size and rank come from get_dist_info()
            # stx()
            ...
            num_iter_per_epoch = math.ceil(
                ...
                (dataset_opt['batch_size_per_gpu'] * opt['world_size']))  # iterations per epoch
            total_iters = int(opt['train']['total_iter'])
            total_epochs = math.ceil(total_iters / (num_iter_per_epoch)) # one iteration = one forward + backward; total_iter is fixed
            logger.info(
                'Training statistics:'
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            j = ((current_iter > groups) != True).nonzero()[
                0]  # index of the current progressive stage
            if len(j) == 0:
                bs_j = len(groups) - 1
            else:
                bs_j = j[0]
            ...
            if mini_batch_size < batch_size:  # the loader yields batch_size pairs; subsample when the stage asks for fewer
            ...
            if opt.get('val') is not None and (current_iter %
                ...
                metric_vals = []
                if hasattr(model, 'metric_results') and model.metric_results:
                    for k in metric_keys:
                        metric_vals.append(model.metric_results.get(k, float('nan')))
                    metric_str = f'{current_iter}' + ''.join([f',{v}' for v in metric_vals])
                else:
                    metric_str = f'{current_iter},{current_metric}'
                logger_metric.info(metric_str)

                # log best metric (explicitly use key_metric, default psnr)
                key_metric = opt['val'].get('key_metric', 'psnr')
                if hasattr(model, 'metric_results') and key_metric in model.metric_results:
                    current_metric = model.metric_results[key_metric]
                if key_metric in best_metric and best_metric[key_metric] < current_metric:
                    best_metric[key_metric] = current_metric
                    best_metric['iter'] = current_iter
                    if hasattr(model, 'metric_results') and model.metric_results:
                        for k in opt['val']['metrics'].keys():
                            if k in model.metric_results:
                                best_metric[k] = model.metric_results[k]
