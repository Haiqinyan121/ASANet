...
class IFA(nn.Module):
    def __init__(self, dim_2, dim, num_heads=2, ffn_expansion_factor=2.66, bias=False,
                 LayerNorm_type='WithBias', use_feature_path=True):
        super(IFA, self).__init__()

        # Fixed Two-Path gate. This is a Python float (not a Parameter/buffer),
        # so it does not change the state_dict or parameter count.
    ...
    def forward(self, input_R, input_S):
        ...
        # F-path gate: suppress only the estimator-produced illumination-conditioned
        # attention contribution, while retaining the IFA main-feature FFN and all parameters.
        attn_out = self.attn(input_R, input_S)
        input_R = input_R + self.feature_gate * attn_out
...
if __name__ == '__main__':
    ...
    # IFA hyperparameters
    dim = 40  # output channels
    num_heads = 8  # attention heads
    ffn_expansion_factor = 2.0  # FFN expansion
    bias = True  # bias in the convolutions
    LayerNorm_type = 'WithBias'  # layer norm with bias
    ...
    # fake inputs for the smoke test
