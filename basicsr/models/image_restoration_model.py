...
class ImageCleanModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        self.apply_trainable_param_patterns(train_opt.get('trainable_param_patterns'))
        ...
        if train_opt.get('pixel_opt'):
            ...
            self.cri_pix_names = []
            if isinstance(pixel_opts, list):
                self.cri_pix = torch.nn.ModuleList()
                for pixel_opt in pixel_opts:
                    pixel_opt = deepcopy(pixel_opt)
                    pixel_type = pixel_opt.pop('type')
                    cri_pix_cls = getattr(loss_module, pixel_type)
                    self.cri_pix.append(cri_pix_cls(**pixel_opt).to(self.device))
                    self.cri_pix_names.append(pixel_type)
            else:
                pixel_opt = deepcopy(pixel_opts)
                pixel_type = pixel_opt.pop('type')
                cri_pix_cls = getattr(loss_module, pixel_type)
    ...
    def apply_trainable_param_patterns(self, patterns):
        if not patterns:
            return
        ...
        for name, param in self.net_g.named_parameters():
            requires_grad = any(pattern in name for pattern in patterns)
            param.requires_grad = requires_grad
            if requires_grad:
                matched += 1

        logger.info(f'Trainable parameter patterns: {patterns}')
        logger.info(f'Trainable parameter tensors: {matched}')

    def setup_optimizers(self):
        train_opt = self.opt['train']
        optim_params = []
        frozen_params = []
        summarize_frozen = bool(train_opt.get('trainable_param_patterns'))
        ...
        for k, v in self.net_g.named_parameters():
            if v.requires_grad:
                ...
                if not summarize_frozen:
                    logger = get_root_logger()
                    logger.warning(f'Params {k} will not be optimized.')

        if summarize_frozen and frozen_params:
            logger = get_root_logger()
            logger.info(f'Frozen parameter tensors: {len(frozen_params)}')
    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast(enabled=self.use_amp):
            ...
            if isinstance(self.cri_pix, torch.nn.ModuleList):
                for pred in preds:
                    for loss_name, cri_pix in zip(self.cri_pix_names, self.cri_pix):
                        current_loss = cri_pix(pred, self.gt)
                        l_pix += current_loss
                        log_name = f'l_pix_{loss_name}'
                        if log_name not in loss_dict:
                            loss_dict[log_name] = current_loss
                        else:
                            loss_dict[log_name] += current_loss
            else:
                for pred in preds:
